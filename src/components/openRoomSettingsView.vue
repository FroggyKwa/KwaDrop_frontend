  ...
  <Modal v-model="dialog" :title="text" @before-open="beforeOpen" @before-close="beforeClose">
    <v-form>
      <div id="form-wrapper">
        ...
        <v-text-field v-model="new_room_password" type="password" color="#56b882" variant="outlined"
                      id="input-room-id"
        ...
        <v-file-input v-model="new_user_avatar" prepend-icon="mdi-alien-outline" chips="true"
                      show-size color="#56b882" variant="outlined" id="input-username"
                      label="Choose your avatar picture"></v-file-input>
        <div class="float-right">
          <v-btn class="button-primary ml-4 mr-4" type="button" @click="onBtnClicked">Ok
            <!--TODO: PATCH-->
          </v-btn>
...
export default defineComponent({
  ...
  methods: {
    ...
    onBtnClicked() {
      ...
      ApiService.renameUser(this.new_user_name);
      if (this.new_user_avatar[0]) {
        ApiService.updateAvatar(this.new_user_avatar[0]);
      }
    },
  },
  components:
